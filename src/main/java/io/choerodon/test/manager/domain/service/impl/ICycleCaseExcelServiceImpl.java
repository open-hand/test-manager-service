...
public class ICycleCaseExcelServiceImpl extends IAbstarctExcelServiceImpl<TestCycleDTO, TestCycleCaseDTO> {
    ...
    public int populateCycleCase(Sheet sheet, int columnNum, TestCycleCaseDTO cycleCase, CellStyle rowStyles) {
        ...
        if(!ObjectUtils.isEmpty(cycleCase.getIssueInfosDTO())) {
            ...
            Optional.ofNullable(getModuleCell(cycleCase.getIssueInfosDTO().getcomponentIssueRelVOList())).ifPresent(v -> ExcelUtil.createCell(row, 7, ExcelUtil.CellType.TEXT, v));
        }
