...
class TestCaseServiceImplSpec extends Specification {
    ...
    def "ListIssueWithoutSub"() {
        when:
        service.listIssueWithoutSub(1L,new SearchDTO(),new PageRequest(sort: new Sort("id")))
        then:
        1*testCaseFeignClient.listIssueWithoutSubToTestComponent(_,_,_,_,_)>>new ResponseEntity<>(new Page(),HttpStatus.OK)
    }

    def "ListIssueWithoutSubDetail"() {
        when:
        service.listIssueWithoutSubDetail(1L,new SearchDTO(),new PageRequest(sort: new Sort("id")))
        then:
        1*testCaseFeignClient.listIssueWithoutSubDetail(_,_,_,_,_)
    }

    def "QueryIssue"() {
        when:
        service.queryIssue(1,2)
        then:
        1*testCaseFeignClient.queryIssue(_,_)
    }

    def "GetIssueInfoMap"() {
        when:
        service.getIssueInfoMap(1L,new SearchDTO(),new PageRequest(sort: new Sort("id")))
        then:
        1*testCaseFeignClient.listIssueWithoutSubToTestComponent(_,_,_,_,_)>>new ResponseEntity<>(new Page(),HttpStatus.OK)
    ...
    def "GetIssueInfoMap1"() {
        ...
        then:
        1*testCaseFeignClient.listIssueWithoutSubDetail(_,_,_,_,_)>>new ResponseEntity<>(new Page(Lists.newArrayList(new IssueComponentDetailDTO(issueId:1L)),new PageInfo(0,1,false),1),HttpStatus.OK)
        when:
    ...
    def "GetIssueInfoMap2"() {
        ...
        then:
        0*testCaseFeignClient.listIssueWithoutSubDetail(_,_,_,_,_)
        0*testCaseFeignClient.listIssueWithoutSubToTestComponent(_,_,_,_,_)
