...
Test manager service of Choerodon.

## Installing the Chart

To install the chart with the release name `test-manager-service`:

```console
$ helm repo add c7n https://openchart.choerodon.com.cn/choerodon/c7n
$ helm repo update
$ helm install test-manager-service c7n/test-manager-service
```
...
| containerPort.actuatorPort | int | `8094` | server management port |
| containerPort.serverPort | int | `8093` | server port |
...
| extraEnv.API_TEST_CLIENT_TEST_RUNNER_IMAGE | string | `"registry.cn-shanghai.aliyuncs.com/c7n/choerodon-test-runner:1.2.0"` |  |
...
| extraEnv.SERVICES_ATTACHMENT_URL | string | `"http://minio.example.com"` |  |
| extraEnv.SERVICES_BUCKET_MERGED | bool | `true` |  |
| extraEnv.SERVICES_FRONT_URL | string | `"http://app.example.com"` |  |
...
| extraEnv.SPRING_DATASOURCE_URL | string | `"jdbc:mysql://localhost:3306/test_manager_service?useUnicode=true&characterEncoding=utf-8&useSSL=false&useInformationSchema=true&remarks=true&allowMultiQueries=true&serverTimezone=Asia/Shanghai"` |  |
| extraEnv.SPRING_DATASOURCE_USERNAME | string | `"username"` |  |
| extraEnv.SPRING_REDIS_DATABASE | int | `13` |  |
...
| image.repository | string | `"c7n/test-manager-service-business"` | service image repository |
...
| service.nodePort | object | `{"actuator":30147,"server":30146}` | Specify the nodePort value for the LoadBalancer and NodePort service types. |
| service.port | object | `{"actuator":8094,"server":8093}` | server Service port |
